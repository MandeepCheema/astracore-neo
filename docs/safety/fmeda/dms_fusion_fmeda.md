...
**Document ID:** ASTR-FMEDA-DMS_FUSION-V0.1  
**Generated:** 2026-04-20  
**Standard:** ISO 26262-5 §7.4.5, §8.4 + ISO 26262-11 §4.6  
**Module:** `rtl/dms_fusion/`  
**ASIL target:** ASIL-D  
...
| Quantity | Value (FIT) |
|---|---:|
| λ_total (all failure modes) | 0.0517 |
| λ_S (safe) | 5.0000e-04 |
| λ_dangerous (DD + DU) | 0.0512 |
| λ_DD (dangerous-detected) | 0.0438 |
| λ_DU (dangerous-undetected, SPF) | 0.0074 |
| λ_LF (latent-fault residual) | 0.0031 |
| λ_DPF (estimated dual-point) | 1.3439e-08 |

## 3. Metrics vs ASIL target

| Metric | Computed | Target (ASIL-D) | Pass? |
|---|---:|---:|:---:|
| SPFM | 85.52 % | ≥ 99 % | ❌ |
| LFM  | 92.99 % | ≥ 90 % | ✅ |
| PMHF | 0.0074 FIT | ≤ 10 FIT | ✅ |

**Overall:** ❌ fails ASIL-D target.

## 4. Findings and next actions

Module-level FMEDA does **not** meet the ASIL-D target (SPFM 85.52 % below target 99 %).

Top dangerous-undetected contributors (drive the SPFM gap):

- `dms_fusion.blink_snapshot.seu` — λ_DU = 0.0020 FIT (Blink window resampling, DC = 70.0 %)
- `dms_fusion.score_filt.seu` — λ_DU = 0.0011 FIT (IIR temporal smoother (self-correcting), DC = 70.0 %)
- `dms_fusion.cont_closed.seu` — λ_DU = 0.0010 FIT (Counter periodic reset, DC = 65.0 %)
